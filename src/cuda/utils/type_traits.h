...
#include "utils/meta_programming.h"  // for detail::TypeTraits, etc.
...
constexpr bool HasTypeTraits = true;
...
//constexpr bool IsCompound = true;
 ...
 */
#if 0
template <class T, class = lad::lcv::cuda::Require<lad::lcv::cuda::IsCompound<T>>>
__host__ std::ostream &operator<<(std::ostream &out, const T &v) {
  using BT = lad::lcv::cuda::BaseType<T>;
  using OutType = std::conditional_t<sizeof(BT) == 1, int, BT>;
  constexpr int NC = lad::lcv::cuda::NumComponents<T>;

  out << lad::lcv::cuda::GetTypeName<T>() << "(";

  for (int c = 0; c < NC; ++c) {
    if (c > 0) {
      out << ", ";
    }
    out << static_cast<OutType>(lad::lcv::cuda::GetElement(v, c));
  }
