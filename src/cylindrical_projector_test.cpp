...
TEST(CylindricalProjector, Operator) {
  FILE *in;
  int length;

  // 1. 打开图片文件
  in = fopen("../../test/resources/bgr_3840_2160_cylindrical_projector_ori.rgb", "rb");
  //in = fopen("../../test/resources/1920_1080.jpg", "rb");
  if (!in) {
    return;
  }
  cudaStream_t stream;
  ...
  lad::lcv::CylindricalProjector cylindrical_projector;
  ...
  int result = cylindrical_projector.Operator(src, dst, lad::lcv::INTERP_NEAREST, 0);
  ...
  FILE *file = fopen("../../test/resources/bgr_3840_2160_cylindrical_projector.rgb", "w");
