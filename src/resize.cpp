...
Resize::Resize(/* args */) { ; }
Resize::~Resize() { ; }
ErrorCode Resize::Operator(const ImageData &src, const ImageData &dst, const InterpolationType interpolation,
                           cudaStream_t stream) {
  ...
  cuda::ResizeOp resize;
  resize.Infer(cu_src, cu_dst, interpolation, stream);
